...
let evenNumbers: number[] = [22, 38, 99, 13, 10, 8, 73, 100];
...
function calculateSum(preValue: number, currValue: number): number {
...
function printMultiTable(number: number, end: number): void {
  for (let i = 1; i <= end; i++) {
    const result = number * i;
    console.log(`${number} × ${i} = ${result}\n`);
  }
...
function printArrayInReverse(arr: number[]): void {
